...
class UploaderController extends AbstractController
{
    ...
    /**
     * @Route("/admin/uploads/transactions/{id}", name="uploader_transaction")
     * @param ConfigFile $file
    ...
    /**
     * @Route("/admin/uploads/transactions/deposit/{id}", name="uploader_transaction_deposit")
     * @param ConfigFile $file
     * @param SaveTransaction $save
     * @return Response
     * @IsGranted("ROLE_SUPER_ADMIN")
     */
    public function uploadDeposit(ConfigFile $file, SaveTransaction $save):Response
    {
        $input = $this->helper->asset($file, 'file');
        $this->reader->setInputFile(substr($input, 1))
            ->setSheetName('POS_TRANSACTIONS')
        ...
        $values = $this->reader->getTransactionValues('CSIN');
        $title = $save->getLine($this->reader);
        $count = 0 ;
        foreach ($values as $key=>$value){
            $save->saveDeposit($save->getValue($value, $title));
            $count ++;
        }
        $this->addFlash('success', "$count enregistrement ont été ajouté dans la base de donnée" );
        $file->setIsLoadDeposit(true);
        $this->manager->persist($file);
        $this->manager->flush();
        return $this->redirectToRoute('transaction_file', [
    ...
    /**
     * @Route("/admin/uploads/transactions/withdrawal/{id}", name="uploader_transaction_withdrawal")
     * @param ConfigFile $file
     * @param SaveTransaction $save
     * @return Response
     * @IsGranted("ROLE_SUPER_ADMIN")
     */
    public function uploadWithdrawal(ConfigFile $file, SaveTransaction $save):Response
    {
        $input = $this->helper->asset($file, 'file');
        $this->reader->setInputFile(substr($input, 1))
            ->setSheetName('POS_TRANSACTIONS')
        ...
        $values = $this->reader->getTransactionValues('AGNT');
        $title = $save->getLine($this->reader);
        $count = 0 ;
        foreach ($values as $key=>$value){
            $save->saveWithdrawal($save->getValue($value, $title));
            $count ++;
        }
        $this->addFlash('success', "$count enregistrement ont été ajouté dans la base de donnée" );
        $file->setIsLoadWithdrawal(true);
        $this->manager->persist($file);
        $this->manager->flush();
        return $this->redirectToRoute('transaction_file', [
    ...
    /**
     * @Route("/admin/uploads/transactions/otherTransaction/{id}", name="uploader_transaction_other")
     * @param ConfigFile $file
     * @param SaveTransaction $save
     * @return Response
     * @IsGranted("ROLE_SUPER_ADMIN")
     */
    public function uploadOtherTransaction(ConfigFile $file, SaveTransaction $save):Response
    {
        $input = $this->helper->asset($file, 'file');
        $this->reader->setInputFile(substr($input, 1))
            ->setSheetName('POS_TRANSACTIONS')
        ...
        $values = $this->reader->getOtherValues();
        $title = $save->getLine($this->reader);
        $count = 0 ;
        foreach ($values as $key=>$value){
            $save->saveOtherTransaction($save->getValue($value, $title));
            $count ++;
        }
        $this->addFlash('success', "$count enregistrement ont été ajouté dans la base de donnée" );
        $file->setIsLoadOther(true);
        $this->manager->persist($file);
        $this->manager->flush();
        return $this->redirectToRoute('transaction_file', [
